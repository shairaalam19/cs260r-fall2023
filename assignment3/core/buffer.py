    ...
    def compute_returns(self, next_value, gamma):
        if self.gae:
            ...
            for step in reversed(range(self.rewards.size(0))):
                ...
                # self.returns[step] = ???
                delta_t = self.rewards[step] + gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
                gae = delta_t + gamma * self.gae_lambda * self.masks[step + 1] * gae
