...
class GAILTrainer(PPOTrainer):
    ...
    def compute_action(self, obs, deterministic=False):
        ...
        if self.discrete:  # Please use categorical distribution.
            ...
            action_log_probs = distribution.log_prob(actions).sum(dim=-1, keepdim=True)
    ...
    def update(self, rollout):
        ...
        for epoch in range(1, self.config.discriminator_epoch + 1):
            ...
            for (expert_data, agent_data) in zip(self.expert_dataloader, data_generator):
                ...
                agent_prediction = self.optimizer_discriminator(agent_generated_obs, agent_generated_actions).flatten()
                expert_prediction = self.optimizer_discriminator(expert_generated_obs, expert_generated_actions).flatten()
                ...
                labels_agent = torch.zeros_like(agent_prediction)
                labels_expert = torch.ones_like(expert_prediction)

                discriminator_loss_agent = discriminator_loss_func(agent_prediction, labels_agent)
                discriminator_loss_expert = discriminator_loss_func(expert_prediction, labels_expert)

                discriminator_loss = discriminator_loss_agent + discriminator_loss_expert
